...
computeSensitivity <- function(viability, concentration, fold.change=3, dilution.no=9, replicates, experiment.dim=c(11, 6), experiment.index.start=3, cutoff=2) {
  viabilityF <- viability
  viability <- read.csv(viability, stringsAsFactors=FALSE, sep="\t", header=FALSE, na.strings=c("", " ", "NA"))
  
  
  concentrationF <- concentration
  concentration <- read.csv(concentrationF, stringsAsFactors=FALSE, sep="\t", na.strings=c("", " ", "NA"))
...
#   }
  ...
  for(index in indicies) {
    ...
    for(j in 1:data.blocks){
      ...
      for(dd in 1:length(drugs)) {
        ...
        fold.change <- concentration$Fold[which(concentration$drug == drug)]
        tt <- as.numeric(doses)
  ...
  sensitivity.profile <- data.frame(matrix(NA, ncol=5, nrow=length(raw.sensitivity.rows), dimnames=list(raw.sensitivity.rows, c("ic50_recomputed", "auc_recomputed", "hill.slope", "einf", "ec50"))))
  for(exp in rownames(raw.sensitivity)) {
    ...
    xx <- unlist(strsplit(gsub(sprintf("_[1-%s]$", 9), "", exp), split='_(?=[^_]+$)', perl=TRUE))
    sensitivity.info[exp, "cell"] <- xx[1]
    sensitivity.info[exp, "drug"] <- xx[2]
    
    sensitivity.profile[exp, "ic50_recomputed"] <- PharmacoGx::computeIC50(concentration=raw.sensitivity.med[exp, , "Dose"], viability=raw.sensitivity.med[exp, , "Viability"])
    sensitivity.profile[exp, "auc_recomputed"] <- PharmacoGx::computeAUC(concentration=raw.sensitivity.med[exp, , "Dose"], viability=raw.sensitivity.med[exp, , "Viability"])
...
save(sensitivityData, file = "/pfs/out/UHNRecomputed_2019.RData")
